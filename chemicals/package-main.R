...
# version for running C code, gradually replace things with fragments of C calls
...
library(funHDDC)
library(mclust)
# nox data made using 6 bsplines for first few and 12 bsplines for last

#******************************* Initialization *******************************#
# TODO: set up the os controlled version
# dyn.load("matrix_funcs.dll")
...
funHDDC1  <-
  function(data, K=1:10, model="AkjBkQkDk", alphamin=c(0.5),threshold=0.1, itermax=200, eps=1e-6,init='random', criterion="bic",
           ...
           d_set=2){
    # IAIN add a option to specify the exact 3 max d values
    ...
    # IAIN add grid to this
    d_select = .myAlerts(d_select, "d_select", "singleCharacterMatch.arg", "HDDC: ", c("cattell", "bic", "grid"))
    ...
    # IAIN check what this step does
    # condition to grid search
    
    if(length(d_set) < max(K)) d_set <- rep(d_set[1], max(K))
    if(d_select=="grid") {
      threshold <- "grid" # not doing cattell
    } else {
      d_set <- rep(2, max(K))
    }
    expand_items = list(model=model, K=K, threshold=threshold)
    for(i in 1:length(d_set)) {
      expand_items[[paste0("d",i)]] <- 2:d_set[i]
    }
    mkt_Expand = do.call(expand.grid, expand_items)
    ...
    # 
    # for(i in 1:max(K)) {
    #   dim_df[[i]] <- data.frame(2:d_set[i])
    #   names(dim_df[[i]]) <- i
    #   if(i > 1) dim_df[[i]] <- merge(dim_df[[i-1]], dim_df[[i]], all=T)
    # }
    # all_dim <- dim_df[[length(dim_df)]]
    # if(d_select != "grid") all_dim <- all_dim[1,]
    # temp_dim <- NULL
    # for(i in 1:nrow(all_dim)) {
    #   if(is.null(temp_dim)) {
    #     temp_dim <- all_dim[i,]
    #   } else {
    #     flag = T
    #     for(j in 2:max(K)) {
    #       if(is.na(all_dim[i, j])) break
    #       if(all_dim[i, j] < all_dim[i, j-1]) {
    #         flag = F
    #       }
    #     }
    #     if(flag) temp_dim <- rbind(temp_dim, all_dim[i,])
    #   }
    #   
    # }
    # temp_dim <- all_dim
    ...
    # IAIN ensure this is flexible
    # mkt_all <- NULL
    # for(ii in 1:nrow(mkt_Expand)) {
    #   tK <- mkt_Expand[ii,]$K
    #   tmodel <- mkt_Expand[ii,]$model
    #   if(tmodel %in% c("AKJBKQKD", "AKBKQKD", "ABKQKD", "AKJBQKD", "AKBQKD", "ABQKD", "AJBQD", "ABQD") | tK == 1) {
    #     use_dim <- all_dim
    #     use_dim[,2:max(K)] <- NA
    #     use_dim <- distinct(use_dim)
    #     if(is.null(mkt_all)) {
    #       mkt_all <- merge(mkt_Expand[ii,], use_dim)
    #     } else {
    #       mkt_all <- rbind(mkt_all, merge(mkt_Expand[ii,], use_dim))
    #     }
    #   } else {
    #     use_dim <- all_dim 
    #     if(tK < max(K)) use_dim[,(tK+1):max(K)] <- NA
    #     use_dim <- distinct(use_dim)
    #     if(is.null(mkt_all)) {
    #       mkt_all <- merge(mkt_Expand[ii,], use_dim)
    #     } else {
    ...
    # IAIN check what this does to the values
    mkt_univariate = apply(mkt_Expand, 1, paste, collapse= "_")
    
    # Mon 'caller' for LTBM that will be used in multi-cores lapply
    # IAIN change to go through combinations of d values (not permutation)
    # IAIN the reason is that group assignments shift for dimension I think
    hddcWrapper = function(mkt_univariate, ...){
      ...
      # IAIN
      threshold = sapply(mkt_splitted, function(x) 
        {
          if(x[3]=="bic"){
            return("bic")
          } else if(x[3]=="grid") {
      ...
      d_set = sapply(mkt_splitted, function(x) { 
        res <- rep(1, K)
        for(i in 1:K) res[i] <- as.numeric(x[3+i])
        return(res)
        })
      ...
      # print(as.vector(d_set))
      try(res <- .funhddc_main1(model=model, K=K, alphamin=alphamin, threshold=threshold, itermax=itermax, d_set=d_set, ...),silent=TRUE)
      res
    }
    ...
    # Display
    # IAIN make display the given d values that it used
    if(show){
      if(n>1) cat("cfunHDDC: \n")
      ...
      d_names <- c()
      myResMat = cbind(model2print[myOrder], K2print[myOrder], thresh2print[myOrder],.addCommas(prms$complexity_allModels[myOrder]), .addCommas(CRIT[myOrder]))
      for(i in 1:max(K)) {
        d2print = as.character(d_keep[[i]])
        d2print = sapply(d2print, function(x) sprintf("%*s", max(nchar(d2print)), x))
      ...
      names(myResMat) = c("model", "K", "threshold", "complexity",toupper(criterion), d_names, "comment")
...
.funhddc_main1 <- function(fdobj, K, model,alphamin, itermax=200,threshold, method, algo, eps, init, init.vector, mini.nb, min.individuals, noise.ctrl, com_dim=NULL, kmeans.control, d_max, d_set, ...){
  # IAIN add taking d values for each group
  # IAIN ? make it so if all are 1 then you use cattell
  l1=length(alphamin)
  ...
  while ((I <- I+1)<=itermax && test>=eps){
    ...
    m <- .funhddc_m_step1(fdobj, K, t,vx,alphax,etax, model,alphamin,threshold, method, noise.ctrl, com_dim, d_max, d_set) # A@5mstep is applied here
  ...
  # IAIN added Q1 as its own thing
  params = list(model=model, K=K, d=d, a=a, b=b, mu=mu, prop=prop,alphax=alphax,etax=etax, ev=m$ev, Q=m$Q, Q1=m$Q1, fpca=m$fpcaobj, loglik=likely[length(likely)], loglik_all = likely, posterior=t, class=cls,outlier=outl, com_ev=com_ev, N=N, complexity=complexity, threshold=threshold, d_select=method, converged=converged) #A25 add alpha and eta
...
.funhddc_e_step1  <- function(fdobj, par,t){ #A@5 this is the E step
  ...
  for (i in 1:K) { #A@5 this must become formula 12,13,14,15 lot's of work
    ...
    # IAIN we send in original Q and a with other stuff
    # exwk = fpcaobj[[i]]$W # IAIN changed
    Qk=Q1[[i]]
    bki=b[i]
    aki = sqrt(diag(c(1/a[i,1:d[i]], rep(1/bki, p-d[i]))))
    # aki=sqrt(diag(1/a[i,1:d[i]],d[i])) IAIN
    muki=mu[i,]
    Wki = chol(par$fpcaobj[[i]]$W)
    # Wki=par$fpcaobj[[i]]$W IAIN
...
.funhddc_m_step1  <- function(fdobj, K, t,vx,alphax,etax, model,alphamin, threshold, method, noise.ctrl, com_dim, d_max, d_set){ #A@5 this is the M step
  if (class(fdobj)!='list') { x = t(fdobj$coefs) #A@5 THIS IS OUR CASE
  } else {x = t(fdobj[[1]]$coefs); for (i in 2:length(fdobj)) x = cbind(x,t(fdobj[[i]]$coefs))}
  ...
  # browser()
  # IAIN change this to not hdclassif just use passed in values
  if (model%in%c("AJBQD", "ABQD")){
    ...
    # IAIN I think we just need to set this to BIC (or not)
    if(threshold=="grid") {
      d <- as.vector(d_set)
    } else {
  ...
  # IAIN this is where they reduce it
  Q1 <- Q
  ...
  # IAIN loop and flatten all wk matrices and Q matrices
  # passing
  # new_t1 <- Sys.time()
  # Qi <- c()
  # Wi <- c()
  # for(i in 1:K) {
  #   Qi <- append(Qi, as.double(Q[[i]]))
  #   Wi <- append(Wi, as.double(fpcaobj[[i]]$W))
  # }
  #etamax = 1000
  # ai_zero <- ai
  # ai_zero[is.na(ai_zero)] <- 0
  # etax_c <- .C("etamax", as.double(t), as.double(vx), as.double(x), 
  #            as.double(mu), as.double(Wi), as.double(Qi), as.double(ai_zero),
  #            as.double(bi), as.integer(N), as.integer(p), as.integer(d),
  #            as.integer(K), eta=as.double(rep(0,K)))$eta
  # start of comment block
  #c_times <<- append(c_times, Sys.time()-new_t1)
  #print(etax_c)
  # new_t2 <- Sys.time()
  ...
  for(i in 1:K){
    ...
    exQki = Q1[[i]]
    # added rep to exa
    exak = sqrt(diag(c(1/ai[i,1:d[i]], rep(1/bi[i], p-d[i]))))
    exbk = bi[i]
    exmuk = matrix(mu[i,], 1, p, byrow=TRUE)
    # exwk = fpcaobj[[i]]$W # IAIN changed
    exwk = chol(fpcaobj[[i]]$W)
    excone=.imahalanobis(x, exmuk, exwk, exQki, exak, exbk)
    # oldcone=.oldRimahalanobis(x, exmuk, fpcaobj[[i]]$W, Q[[i]], 
    #                           sqrt(diag(c(1/ai[i,1:d[i]]))), exbk)
    #box_res[["triangle"]] <<- append(box_res[["triangle"]], (excone-oldcone)/oldcone)
    #print(paste0("imahSum ",sum(exz*(1.0-exv)*excone) ))
    #print(paste0("No imahSum ",sum(exz*(1.0-exv)) )
    ala<-sum(exz*(1.0-exv))
    blb<-sum(exz*(1.0-exv)*excone)
    etry<-blb/(p*ala)
    #print(paste0(blb,"/(",p,"*",ala,")"))
    etamax=1000
  ...
  #r_times <<- append(r_times, Sys.time()-new_t2)
  #print(etax)
  #print(etax - etax_c)
  ...
  # IAIN change the passed in Q
  list(model=model, K=K, d=d, a=ai, b=bi, mu=mu, prop=prop, alphax=alphax,etax=etax,ev=ev, Q=Q, Q1=Q1, fpcaobj = fpcaobj) #A@5 add the extra params(alpha and eta)
...
.mypca.fd1 <- function(fdobj, Ti,CorI){
  if (class(fdobj)=="list"){ #A@5 NOT OUR CASE 
    ...
    for (i in 1:length(fdobj)){
      # coefmean <- apply(t(as.matrix(Ti) %*% matrix(1,1,nrow(fdobj[[i]]$coefs))) *
      #                     fdobj[[i]]$coefs, 1, sum) / sum(Ti) 
      coefmean <- apply(t(as.matrix(CorI) %*% matrix(1,1,nrow(fdobj[[i]]$coefs))) *
    ...
    # mat_cov <- crossprod(t(.repmat(sqrt(Ti),n=dim(t(coef))[[1]],p=1)*
    #                          t(coef)))/sum(Ti) 
    mat_cov <- crossprod(t(.repmat(sqrt(CorI),n=dim(t(coef))[[1]],p=1)*
                             t(coef)))/sum(Ti) 
    ...
    coefmean <- apply(t(as.matrix(CorI) %*% matrix(1,1,nrow(fdobj$coefs))) * 
                        fdobj$coefs, 1, sum) / sum(CorI) #A@5 here they compute miu(i) - second line of fomula 19
    # coefmean <- apply(t(as.matrix(Ti) %*% matrix(1,1,nrow(fdobj$coefs))) * 
    #                   fdobj$coefs, 1, sum) / sum(Ti) #A@5 here they compute miu(i) - second line of fomula 19
...
.hdclassif_bic <- function(par, p, data=NULL){
  ...
  if (is.null(par$loglik)){
    ...
    # IAIN do we use diag(c(a, b)) here
...
# original version of mahalanobis
#imahalanobis(x, exmuk, exwk, exQki, exak, exbk)
.Rimahalanobis <- function(x, muk, wk, Qk, aki, bki) {
  # w = fpcaobj[[i]]$W
  # *************************************************************************** #
  ...
  # sum of the Sfirst two terms is new mahalanobis
  ...
  #   a_side = 0
  #   b_side = 0
  #   #for(j in 0:(nrow(aki)-1)) a_side = a_side + A[j*N+i+1]*A[j*N+i+1]
  #   for(j in 0:(nrow(aki)-1)) a_side = a_side + A[i+1,j+1]*A[i+1,j+1]
  #   #for(j in 0:(p-1)) b_side = b_side + B[j*N+i+1]*B[j*N+i+1]
  #   for(j in 0:(p-1)) b_side = b_side + B[i+1,j+1]*B[i+1,j+1]
  #   res[i+1] = a_side + b_side/bki
  # }
  #print(res_old-res)
  #print(sprintf("%.80f", res_old))
  #print(sprintf("%.80f", res))
  return(res_old)
}

.oldRimahalanobis <- function(x, muk, wk, Qk, aki, bki) {
  # w = fpcaobj[[i]]$W
  # *************************************************************************** #
  # should be called as imahalanobis(x, mu[i,], w[i,], Q[[i]], a[i,], d[i], b[i])
  # return formula on top of page 9
  ...
  # sum of the Sfirst two terms is new mahalanobis
  #return(rowSums(A^2)+1/bk[i]*rowSums(B^2))
  res_old<-.rowSums(A^2, N, nrow(aki))+.rowSums(B^2, N, p)/bki
  # res <- rep(0,N)
  # for(i in 0:(N-1)) {
  #   a_side = 0
  #   b_side = 0
...
# testing run with slow C version
.Cimahalanobis <- function(x, muk, wk, Qk, aki, bki) {
  N <- nrow(x)
  p <- ncol(x)
  a <- nrow(aki)
  
  dyn.load("matrix_funcs.so")
  time1_c <- Sys.time()
  res <- .C("C_imahalanobis", as.double(x), as.double(muk), as.double(wk),
            as.double(Qk), as.double(aki), as.double(bki), as.integer(p),
            as.integer(N), as.integer(a), res = rep(0,N))
  time2_c <- Sys.time()
  res1 <- res$res
  # remove this eventually
  time1_r <- Sys.time()
  res2 <- .Rimahalanobis(x, muk, wk, Qk, aki, bki)
  time2_r <- Sys.time()
  
  time1_slowc <- Sys.time()
  res <- .C("imahalanobis", as.double(x), as.double(muk), as.double(wk),
            as.double(Qk), as.double(aki), as.double(bki), as.integer(p),
            as.integer(N), as.integer(a), res = rep(0,N), 
            X = rep(0,N*p), Qi = rep(0, a*p), proj = rep(0, N*p), 
            A = rep(0, N*a), B = rep(0, N*p))
  time2_slowc <- Sys.time()
  print(paste0("C1 and C2 diff: ",sqrt(sum((res$res - res1)^2))))
  if(sqrt(sum((res$res - res1)^2)) > 0.0001) stop("Oh No...")
  # print result times
  print(paste0("C Time: ", time2_c-time1_c))
  print(paste0("Slow C Time: ", time2_slowc-time1_slowc))
  #c_times <<- append(c_times, time2_c-time1_c)
  slowc_times <<- append(slowc_times, time2_slowc-time1_slowc)
  print(paste0("R Time: ", time2_r-time1_r))
  #r_times <<- append(r_times, time2_r-time1_r)
  if(sqrt(sum((res1 - res2)^2)) < 0.0001){
    print("GOOD")
    print(sqrt(sum((res1 - res2)^2)))
  } else {
    print("BAD")
    print(res1)
    print(res2)
  ...
  dyn.unload("matrix_funcs.so")
  return(res$res)
}

# Final fast implementation of C version
# IAIN change to .imahalanobis after completion
.C_imahalanobis <- function(x, muk, wk, Qk, aki, bki) {
  N <- nrow(x)
  p <- ncol(x)
  a <- nrow(aki)
...
#******************************** Testing Code ********************************#
# run with different sizes too
run_mihalanobis <- function(N=9, p=3, a=2, div_factor=10) {
  # declare matrices to use, simple version
  x <- matrix((1:(N*p))/div_factor,nrow=N, ncol=p, byrow = T)# data Nxp
  muk <- colMeans(x) # mus 1xp
  # wk <- diag(rep(1,p)) # weights pxp diagonal -> start as a diagonal identity matrix
  wk <- matrix((1:(p*p))/div_factor, nrow=p, ncol=p)
  Qk <- matrix((1:(p*a))/div_factor, nrow=p, ncol=a, byrow = T) # px? (13 in trial run)
  aki <- diag(rep(2,a)) # ?x? (13x13) diagonal matrix
  bki <- 4 # 1x1
  # basic imahalanobis run 
  Rmihalanobis <- .Rimahalanobis(x,muk,wk,Qk,aki,bki)
  # run the cmihalanobis
  dyn.load("matrix_funcs.dll")
  # there is an issue with C_imihalanobis and I know it exists and where it is can be seen with N=12,p=4,a=3
  # increasing it seemed to work but if it crashes on that then something is wrong
  res <- .C("imahalanobis", as.double(x), as.double(muk), as.double(wk),
     as.double(Qk), as.double(aki), as.double(bki), as.integer(p),
     as.integer(N), as.integer(a), res = rep(0,N), 
     X = rep(0,N*p), Qi = rep(0, a*p), proj = rep(0, N*p), 
     A = rep(0, N*a), B = rep(0, N*p))
  Cmihalanobis <- res$res
  # a <<- res
  
  res_f <- .C("C_imahalanobis", as.double(x), as.double(muk), as.double(wk),
              as.double(Qk), as.double(aki), as.double(bki), as.integer(p),
              as.integer(N), as.integer(a), res = rep(0,N))
  
  C2imahalanobis <- res_f$res
  print("C_ vs C")
  print(sqrt(sum((C2imahalanobis - Cmihalanobis)^2)))
  print("R vs C_")
  print(sqrt(sum((Rmihalanobis - C2imahalanobis)^2)))
  print("R vs C")
  print(sqrt(sum((Rmihalanobis - Cmihalanobis)^2)))
  
  dyn.unload("matrix_funcs.dll")
}

# fun_RvC_comparison(wed$fd, wed$labels, K=4, init="kmeans", nb.rep=10, itermax=100)
fun_RvC_comparison <- function(in_fd, a_clust, ...) {
  # run both versions on the data and get results
  .imahalanobis <<- .Rimahalanobis
  set.seed(10403)
  res_R <- funHDDC1(in_fd,...)
  # code can be added here for matrix checks
  m_val_R <- m_val_C
  e_val_R <- e_val_C
  
  .imahalanobis <<- .C_imahalanobis
  set.seed(10403)
  res_C <- funHDDC1(in_fd,...)
  # code can be added here for matrix checks
  m_val_C_ <- m_val_C
  e_val_C_ <- e_val_C
  
  # get ari for both of the results
  ariRC <- adjustedRandIndex(res_R$class, res_C$class)
  ariR <- adjustedRandIndex(a_clust, res_R$class)
  ariC <- adjustedRandIndex(a_clust, res_C$class)
  
  ariOutRC <- adjustedRandIndex(res_R$outlier, res_C$outlier)
  
  # reset .imahalanobis
  .imahalanobis <<- .C_imahalanobis
  
  # return final results
  return(list(ariRC=ariRC, ariR=ariR, ariC=ariC, ariOutRC=ariOutRC,
              m_val_R=m_val_R, e_val_R=e_val_R,
              m_val_C=m_val_C_, e_val_C=e_val_C_,
              res_R=res_R, res_C=res_C))
}

print_EM_res_diff <- function(e_val_1, e_val_2, m_val_1, m_val_2) {
  box_list <- list()
  box_list$ev <- (m_val_1$ev - m_val_2$ev)/m_val_1$ev
  print(paste0("Max EV diff: ", max(abs(box_list$ev), na.rm=T)))
  box_list$a <- (m_val_1$a - m_val_2$a)/m_val_1$a
  print(paste0("Max a diff: ", max(abs(box_list$a), na.rm=T)))
  box_list$mu <- (m_val_1$mu - m_val_2$mu)/m_val_1$mu
  print(paste0("Max mu diff: ", max(abs(box_list$mu), na.rm=T)))
  for(i in 1:length(m_val_1$Q)) {
    box_list[[paste0("Q",i)]] <- (m_val_1$Q[[i]] - m_val_2$Q[[i]])/m_val_1$Q[[i]]
    print(paste0("Max Q", i, " diff: ", max(abs(box_list[[paste0("Q",i)]]), na.rm=T)))
  }
  box_list$t <- (e_val_1$t - e_val_2$t)/e_val_1$t
  print(paste0("Max t diff: ", max(abs(box_list$t), na.rm=T)))
  box_list$vx <- (e_val_1$vx - e_val_2$vx)/e_val_1$vx
  print(paste0("Max vx diff: ", max(abs(box_list$vx), na.rm=T)))
  print(paste0("L diff: ", abs((e_val_1$L - e_val_2$L)/e_val_1$L)))
...
# example call of full comparison code
# make sure to change seed for each contamination scheme
# set.seed(10403) # use a standard seed for generating data
# s_fd1 <- simulate_fd(eta=c(7,10,17))
# fun_RvC_comparison(s_fd1$fd, s_fd1$fd$class, K=3, model=models, 
#                    init="kmeans", threshold=0.2, itermax=200, nb.rep=50, 
#                    alphamin=0.75)
# set.seed(17581) # for 2
# set.seed(25431) # for 3
# print_EM_res_diff(res_1$e_val_R, res_1$e_val_C, res_1$m_val_R, res_1$m_val_C)

# basis <- create.fourier.basis(rangeval = c(0,23), nbasis = 8)
# fds <- smooth.basis(argvals = seq(0, 23, length.out = 24), y=t(poblenou$nox$data), fdParobj = basis)$fd


# .C("etamax", as.double(exz), as.double(exv), as.double(x), as.double(exmuk), as.double(exwk), as.double(exQki), as.double(exak), as.double(exbk), as.double(excone), as.integer(N), as.integer(p), as.integer(d[i]), as.double(0))
#time_grid <- Sys.time()
# example of running with a grid
#res12 <- funHDDC1(nox_fd$fd, K=2, alphamin = 0.5, d_select = "grid", d_set = 10, nb.rep = 10, itermax = 200, model=models)
#time_grid <- Sys.time() - time_grid
# time_bic <- Sys.time()
# res11 <- funHDDC1(nox_fd$fd, K=2, alphamin = 0.75, d_select = "bic", d_set = 3, nb.rep = 10, itermax = 200, model=models)
# time_bic <- Sys.time() - time_bic

#set.seed(1403)
#res <- funHDDC1(nox_fd$fd, K=2, alphamin = 0.8, d_select = "grid", d_set = 2, nb.rep = 1, model=models, init="kmeans")
