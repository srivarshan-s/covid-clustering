...
hddc_chem <- function(init="random", nbasis=10) {
  ...
  thresh <- c(0.001,0.01,0.05,0.1,0.2,0.3)
...
thddc_chem <- function(init="random", nbasis=10) {
  trig <- merge_chemical_csv(nbasis=nbasis, allowed_classes=1:10, dim_use = c(3, 5, 9))
  trig$class <- trig$groupd
  trig$groupd <- ifelse(trig$groupd==1 | trig$groupd==10, 
                        as.character(trig$groupd), "other")
...
get_original_data_pca <- function(components=10) {
  chemical_labels <- read.csv("./Raw_CSV/chemical_index.csv")
  chemical_class <- c()
  full_df <- NULL
  for(load_number in 1:65) {
    load_name <- paste0("./Raw_CSV/", load_number, ".csv")
    load_df <- read.csv(load_name)
    if(is.null(full_df)) {
      full_df <- load_df[2:nrow(load_df),]
    } else {
      full_df <- rbind(full_df, load_df[2:nrow(load_df),])
    }
    chemical_class <- 
      append(chemical_class, rep(chemical_labels$Class[load_number], nrow(load_df)-1))
  }
  
  full_df$class <- ifelse(chemical_class==1 | chemical_class==10, 
                          as.character(chemical_class), "other")
  
  # hard part of the problem is that the times do not line up so how do I do
  # this part?
  
  # run the pca
  pca_df <- prcomp(full_df, retx = T, scale. = T, center = T)
}
...
# print("HDDC")
# sink("hddc_10spline.txt")
# hddc_chem(init="kmeans", nbasis=10)
# sink()
# print("THDDC")
# sink("thddc_4spline_tkmeans.txt")
# thddc_chem(init="tkmeans", nbasis=4)
# sink()
# print("THDDC")
# sink("thddc_10spline.txt")
# thddc_chem(init="kmeans", nbasis=10)
# sink()
# print("THDDC")
# sink("thddc_10spline_tkmeans.txt")
# thddc_chem(init="tkmeans", nbasis=10)
# sink()
# print("CHDDC")
# sink("chddc_10spline.txt")
