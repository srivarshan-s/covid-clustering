...
.tfunhddc_main1 <- function(fdobj, K, dfstart=dfstart,dfupdate=dfupdate,dconstr=dconstr,model, itermax=200,threshold, method, algo, eps=1e-6, init, init.vector, mini.nb, min.individuals, noise.ctrl, com_dim=NULL, kmeans.control, d_max, ...){
  ...
  params = list(model=model, K=K, d=d, a=a, b=b, mu=mu, prop=prop,nux=nux, ev=m$ev, Q=m$Q, Q1=m$Q1, fpca=m$fpcaobj, loglik=likely[length(likely)], loglik_all = likely, posterior=t, class=cls, com_ev=com_ev, N=N, complexity=complexity, threshold=threshold, d_select=method, converged=converged) #A25 add alpha and eta
...
.funhddt_init  <- function(fdobj, K,t,nux, model, threshold, method, noise.ctrl, com_dim, d_max){ #A@5 this is the init step
  ...
  list(model=model, K=K, d=d, a=ai, b=bi, mu=mu, prop=prop, ev=ev, Q=Q,
       fpcaobj=fpcaobj, Q1=Q1) #A@5 add the extra params(alpha and eta)
...
.initmypca.fd1 <- function(fdobj, Ti){
  if (class(fdobj)=="list"){ #A@5 NOT OUR CASE 
    ...
    # IAIN this here should be using both of the sets together
    ...
    fdobj[[1]]$coefs <- sweep(fdobj[[1]]$coefs, 1, coefmean[(n_lead+1):(n_var+n_lead)])
    ...
    ipcafd<-list(valeurs_propres=valeurs_propres,harmonic=fonctionspropres,scores=scores,covariance=cov,U=bj,varprop=varprop,meanfd=mean_fd,W=W_tot, mux=coefmean)
...
.funhddt_twinit  <- function(fdobj, par,nux){ #A@5 this is the E step
  ...
  for (i in 1:K) { #A@5 this must become formula 12,13,14,15 lot's of work
    ...
    #Qk=Q[[i]]
    Qk=Q1[[i]]
    #aki=sqrt(diag(c(1/a[i,1:d[i]], d[i]))
...
.funhddt_m_step1  <- function(fdobj, K, t,tw,nux,dfupdate, dconstr,model, threshold, method, noise.ctrl, com_dim, d_max){ #A@5 this is the M step
  ...
  list(model=model, K=K, d=d, a=ai, b=bi, mu=mu, prop=prop, nux=nux,ev=ev, Q=Q,
       fpcaobj=fpcaobj, Q1=Q1) #A@5 add the extra params(alpha and eta)
...
.mypcat.fd1 <- function(fdobj, Ti,CorI){
  if (class(fdobj)=="list"){ #A@5 NOT OUR CASE 
    ...
    for (i in 1:length(fdobj)){
      ...
      coefmean <- apply(t(as.matrix(CorI) %*% matrix(1,1,nrow(fdobj[[i]]$coefs))) * fdobj[[i]]$coefs, 1, sum) / sum(CorI) 
      fdobj[[i]]$coefs <- sweep(fdobj[[i]]$coefs, 1, coefmean)  
      mean_fd[[i]]$coefs = as.matrix(data.frame(mean=coefmean)) 
    }
...
auto_run <- function(input_data, method, seeds = c(5477, 10403, 15971, 30479), init="kmeans") {
  res <- list()
  # threshes <- c(0.01)
  threshes <- c(0.01, 0.05, 0.1, 0.2)
  models <- c( 'AkjBkQkDk', 'AkjBQkDk', 'AkBkQkDk', 'ABkQkDk', 'AkBQkDk', 'ABQkDk')
  for(i in 1:length(threshes)) {
    thresh <- threshes[i]
    if(method == "funHDDC") {
      set.seed(seeds[i])
      res[[paste0(method, " t", thresh)]] <- 
        funHDDC(input_data, K = 2:4, model = models, threshold = threshes[i], 
                itermax = 200, nb.rep = 50, init = init)
      res[[paste0(method, " t", thresh)]]$rownames <- input_data[[1]]$rownames
      res[[paste0(method, " t", thresh)]]$outliers <- input_data[[1]]$outliers
    } else if (method == "cfunHDDC") {
      set.seed(seeds[i])
      res[[paste0(method, " t", thresh, " a", "0.75")]] <- 
        funHDDC1(input_data, K = 2:4, model = models, threshold = threshes[i], 
                 itermax = 200, nb.rep = 50, init = init, alphamin=0.75)
      res[[paste0(method, " t", thresh, " a", "0.75")]]$rownames <- input_data[[1]]$rownames
      res[[paste0(method, " t", thresh, " a", "0.75")]]$outliers <- input_data[[1]]$outliers
    } else if (method == "tfunHDDC") {
      res[[paste0(method, " t", thresh)]] <- 
        tfunHDDC(input_data, K = 2:4, model = models, threshold = threshes[i], 
                 itermax = 200, nb.rep = 50, init = init, 
                 dfupdate = "numeric", dconstr = "no")
      res[[paste0(method, " t", thresh)]]$rownames <- input_data[[1]]$rownames
      res[[paste0(method, " t", thresh)]]$outliers <- input_data[[1]]$outliers
    } else {
      stop("Not a recognized method.")
    }
  }
  return(res)
}

#res_funHDDC24_multi30 <- auto_run(all_fd, "funHDDC", init = "kmeans", seeds = rep(30479, 4))
#save(res_funHDDC24_multi30, file = "res_funHDDC24_multi30.Rdata")
#res_cfunHDDC75_24_multi30 <- auto_run(all_fd, "cfunHDDC", init = "kmeans", seeds = rep(30479, 4))
#save(res_cfunHDDC75_24_multi30, file = "res_cfunHDDC50_24_multi30.Rdata")
#res_tfunHDDCYes_24_multi30 <- auto_run(fdata_cases, "tfunHDDC", init = "kmeans", seeds = rep(30479, 4))
#save(res_tfunHDDCYes_24_multi30, file = "res_tfunHDDCNo_24_multi30.Rdata")

# get the trim=0.1 outliers from the data
# everything is done calculated and plotted in a simple manner to grow on

# find if dconstr yes or no works better

# run all 3 methods and plot their clusterings as individual groups w/ outs
