...
cluster_helper <- function(method, fd, alphamin,itermax, ...) {
  x <- list(...) 
  models = c("EII", "VII", "EEI", "VEI", "EVI", "VVI", "EEE", "VEE", "VVE", "VEV", "EVV", "VVV")
  print(method)
  switch (method,
          funHDDC = return(funHDDC(fd,itermax=itermax,...)),
          cfunHDDC = return(funHDDC1(fd, alphamin = alphamin,itermax=itermax, ...))
#CNmixt = return(CNmixt(t(fd$coefs),G=x$K, contamination = T, 
 #                                initialization = x$init, model=models, alphamin=alphamin))
  )
}
get_classification <- function(cclust, cmname) {
  print(paste0("Retrieving Best Result From: ", cmname))
  if(cmname=="CNmixt") best <- getBestModel(cclust, criterion="BIC")
  switch (cmname,
          funHDDC = return(cclust$class),
          cfunHDDC = return(cclust$class),
          CNmixt = return(getCluster(best))
  )
}


cluster_comparison <- function(ntests, K=3, clust_FUN=cluster_helper, 
                               fun_names=c("cfunHDDC","funHDDC"), 
                               rseeds=NULL, nrep=20, 
                               eta=c(15, 5, 50), alpha=c(0.9,0.9,0.9), 
                               ncurves=1000, nsplines=35, alphamin=c(0.5), threshold=0.1) {
  ####################################################################################
  ...
  names(classifications) <- c("n", "seed", "method", "model", "ari", "CCRout", 
                              "ariout",
                              "eta1","eta2","eta3",
                              "alpha1","alpha2","alpha3") # rename columns
  # prefix for csvs
  ...
  fullclass <- foreach(i = 1:ntests,.packages = c("funHDDC", "fda",
    ...
    fdlist <- simulate_fd(ncurves=ncurves, nsplines=nsplines, 
                          eta = eta, alpha = alpha)
    ...
    for(j in 1:length(fun_names)) { # get clustering results for each method
      ...
      if(!is.null(get_classification(cclust, fun_names[j]))) { # check if the clustering was successful
        ...
        }else if(fun_names[j]=="CNmixt") {
	  best <- getBestModel(cclust, criterion="BIC")
          label <- ifelse(getDetection(best)$innergroup=="*", 0, 1)
          outtable <- table(fdlist$simdata$clo, label)
          outdet <- (outtable[2] + outtable[3])/sum(outtable)
          outari <- adjustedRandIndex(fdlist$simdata$clo, label)
          current_class <- rbind(current_class, 
                                  c(i, seed_clust, 
                                    fun_names[j], best$models[[1]]$model,
                                    adjustedRandIndex(classes[[j]], 
                                                      fdlist$simdata$clx),
                                    outdet, outari, best$models[[1]]$eta,
                                    best$models[[1]]$alpha))
...
cluster_comparison(100, eta=c(5, 50, 15), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600], nsplines=35, nrep=50, alphamin = 0.75, threshold=0.05)
