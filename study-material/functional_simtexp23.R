#********************************* RUNNING SIMS *********************************#
# cluster_comparison(1, fun_names=c("cfunHDDC"), nrep=10, eta=c(1.2, 12, 5), nsplines=35, ncurves=1000, alpha=c(0.9, 0.9, 0.9))
#ianos<-cluster_comparison(100, fun_names=c("cfunHDDC", "funHDDC"), nrep=50, eta=c(5, 15, 50), nsplines=70, ncurves=1000, alpha=c(0.9, 0.9, 0.9))
#********************************* INSTALL REQUIRED LIBRARIES *********************************#
#install.packages("funHDDC", repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
#install.packages("ContaminatedMixt", repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
...
#setwd("~/projects/def-popescuc/popescuc")
...
newContaminatedSimulation <- function(ncurves, p, mu, K=3, prop=NULL, 
  ...
  ####################################################################################
  #IS i think p is the multivariate nature so we use this as # coefficients
  ...
  #IS this should be able to stay the same
  ...
  # IS get the eta diagonal
  # etadiag <- vector(mode='list', length=K)
  # for (i in 1:K) etadiag[[i]] <- diag(ifelse(runif(0,1, n=p) > alpha[i], eta[i], 1))
  #for (i in 1:K) etadiag[[i]] <- diag(rep(1, p)) # test for before contamination
  #IS added eta[i] to define the contamination within groups
  for (i in 1:K)	{
  ...
  cls<-X<-NULL
  clo <- rep(1, N)
  #IS contaminate sigma only on certain rows and all sigma for that
  ...
  return(list(X=X, clx=cls, clo=clo, prms=prms))
  
}
# create the mu matrix

simulate_fd <- function(ncurves=1000, nsplines=35, 
  ...
  fds <- smooth.basis(argvals = seq(0, 100, length.out = 100), y=t(finaldata[,1:100]), fdParobj = bbasis)$fd
  fds$class <- finaldata[,101]
  return(list(fd=fds, simdata=coef))
}


cluster_helper <- function(method, fd,alphamin,itermax,dfstart,dfupdate,dconstr, ...) {
  x <- list(...)
  print(method)
  switch (method,
          funHDDC = return(funHDDC(fd,itermax=itermax,...)),
          cfunHDDC = return(funHDDC1(fd, alphamin = alphamin,itermax=itermax, ...)),
          tfunHDDC  = return(tfunHDDC(fd,dfstart=dfstart,dfupdate=dfupdate, dconstr=dconstr, ...))
  )
}

get_classification <- function(cclust, cmname) {
  print(paste0("Retrieving Best Result From: ", cmname))
  switch (cmname,
         funHDDC = return(cclust$class),
          tfunHDDC = return(cclust$class),
...
get_d <- function(cclust, cmname) {
  switch (cmname,
          ...
          tfunHDDC = return(cclust$d),
          cfunHDDC = return(cclust$d)
  )
}
get_nux <- function(cclust, cmname) {
  switch (cmname,
          funHDDC = return(0),
          tfunHDDC = return(cclust$nux),
          cfunHDDC = return(0)
  )
}

cluster_comparison <- function(ntests, K=3, clust_FUN=cluster_helper, 
                               fun_names=c("tfunHDDC"), 
                               rseeds=NULL, nrep=20, 
                               eta=c(15, 5, 50), alpha=c(0.9,0.9,0.9), 
                               ncurves=1000, nsplines=35, alphamin=c(0.5), threshold=0.1,init='kmeans',dfstart=50,dfupdate="numeric",dconstr="no") {
  ####################################################################################
  # Input: ntests(int) -> number of simulations and clusterings to run               #
  #        K(vector) : default 3:6 -> group sizes funHDDC can use                    #
  #        clust_FUN : default c(funHDDC) -> vector of clustering functions          #
  #        rseeds : default .Random.seed -> list of seeds to use for tests           #
  # Purpose: For each of ntests get a random seed to use, produce simulated          #
  #  contaminated data and cluster it using funHDDC on kmeans. Then from the results #
  #  produce the mclust table and extract information about amount of contamination  #
  #  clustered together and amount of information that was misclassified. Save each  #
  #  cluster into a file and save the table information.                             #
  # Output: (list) -> $bm -> break down of models chosen by funHDDC.                 #
  #                   $am -> all cluster sizes chosen for each group.                #
  #                   $ntcg -> number of contaminated curves in the most             #
  #                            contaminated group.                                   #
  #                   $nextra -> number of missclassified curves total for each test #
  #                              missclassified being not the majority of curves in  #
  #                              the group.                                          #
  #                   $nusplit -> number of uncontaminated curves split from their   #
  #                               original groups.                                   #
  #                   $ncontam -> number of contaminated curves total.               #
  #                   $seeds -> seed set for each tests for reproducing results.     #
  #                   $simulations -> simulations for each test.                     #
  #                   $clusters -> cluster results for each test.                    #
  #                   $tables -> mclust tables for each test.                        #
  ####################################################################################
  # set up containers to hold results
  classifications <- data.frame(matrix(ncol = 13,nrow=0))
  names(classifications) <- c("n", "seed", "method", "model", "ari", "CCRout", 
                              "ariout",
                              "eta1","eta2","eta3",
                              "alpha1Nux1","alpha2Nux2","alpha3Nux3") # rename columns
  # prefix for csvs
  csv_prefix <- paste0("n_", ntests, "_K_", min(K), "-", max(K), 
                       "_nm_", length(fun_names), "_nrep_", nrep, 
                       "_eta_", eta[1], "-", eta[2],  "-", eta[3],"-", threshold)
  fullclass <- foreach(i = 1:ntests,.packages = c("funHDDC", "fda",
                                                  "mclust"), .combine=rbind, .init=NULL, .errorhandling="remove") %dopar% {
                                                    
                                                    
    classes <- list()
    current_class <- data.frame(matrix(ncol = 13,nrow=0))
    seed_fds <- rseeds[i]
    set.seed(seed_fds)
    # produce functional data
    fdlist <- simulate_fd(ncurves=ncurves, nsplines=nsplines,eta = eta, alpha = alpha)
    # save the grouping data
    current_class <- rbind(current_class, c(i, seed_fds, "true", NA, NA, NA, NA,
                                            eta, alpha))
    # set the seed that will be used for clustering
    seed_clust <- rseeds[i+ntests]
    for(j in 1:length(fun_names)) { # get clustering results for each method
      set.seed(seed_clust) # reset the seed
      # get clustering results
      cclust <- clust_FUN(fun_names[j], alphamin, threshold=threshold, fd=fdlist$fd, K=K, nb.rep=nrep,dfstart=dfstart,dfupdate=dfupdate,dconstr=dconstr,
                          init='kmeans', itermax=200,model=c('AkjBkQkDk', 'AkjBQkDk', 
                                                             'AkBkQkDk', 'ABkQkDk', 
                                                             'AkBQkDk', 'ABQkDk'))
      
     # cclust <- clust_FUN(fun_names[j], alphamin, threshold=threshold, fd=fdlist$fd, K=K, nb.rep=nrep,
      #                    init='kmeans', itermax=200,model=c('AkjBkQkDk', 'AkjBQkDk', 
       #                                          'AkBkQkDk', 'ABkQkDk', 
        #                                         'AkBQkDk', 'ABQkDk'),threshold=threshold,dfstart=dfstart,dfupdate=dfupdate,dconstr=dconstr)
      
      if(!is.null(get_classification(cclust, fun_names[j]))) { # check if the clustering was successful
        # save classifications
        classes[[j]] <- get_classification(cclust, fun_names[j])
        if(fun_names[j]=="cfunHDDC") {
          outtable <- table(fdlist$simdata$clo, cclust$outlier)
          outdet <- (outtable[1] + outtable[4])/sum(outtable)
          outari <- adjustedRandIndex(fdlist$simdata$clo, cclust$outlier)
          current_class <- rbind(current_class, 
                                 c(i, seed_clust, 
                                   fun_names[j], cclust$model,
                                   adjustedRandIndex(classes[[j]], fdlist$simdata$clx),
                                   outdet, outari, cclust$etax,cclust$alphax))
        }else if(fun_names[j]=="tfunHDDC") {
          outdet <- NA
          outari <- NA
          current_class <- rbind(current_class, 
                                 c(i, seed_clust, 
                                   fun_names[j], cclust$model,
                                   adjustedRandIndex(classes[[j]], fdlist$simdata$clx),
                                   outdet, outari, rep(NA, 3),cclust$nux))
...
r_seeds <- read.csv(file="random_seeds.csv")$x
#cluster_comparison(100, eta=c(1, 1, 1), alpha=c(1,1,1), rseeds=r_seeds[1:200], nsplines=70, nrep=50, alphamin = c(0.75))
#cluster_comparison(100, eta=c(100, 70, 170), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600], nsplines=35, nrep=50, alphamin = 0.75, threshold=0.1)
#cluster_comparison(100, eta=c(5, 50, 15), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600], nsplines=35, nrep=50, alphamin = 0.75, threshold=0.1)
cluster_comparison(100, eta=c(5, 50, 15), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600], nsplines=35, nrep=50, alphamin = 0.75, threshold=0.2,dfstart=50,dfupdate="numeric", dconstr="no")
#cluster_comparison(2, K=3,eta=c(100, 70, 170), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600],
 #                nsplines=35, nrep=1, alphamin = 0.75, fun_names=c("tfunHDDC"),threshold=0.2,init="kmeans",dfstart=50,dfupdate="numeric", dconstr="no")
#cluster_comparison(100, eta=c(5, 2, 3), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[401:600], nsplines=70, nrep=50, alphamin = 0.80, etamax = 10)
#cluster_comparison(100, eta=c(15, 5, 50), alpha=c(0.9,0.9,0.9), rseeds=r_seeds[601:800], nsplines=70, nrep=50, alphamin = 0.85, etamax = 70)
